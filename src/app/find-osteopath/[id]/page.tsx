...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  return getLocalizedMetadata(path, profile ? {
    ...
    ar: { title: `${profile.name} · دليل EGSOM`, description: `${profile.name}، ${profile.specialtyAr || profile.specialty}، ${profile.city}، ${profile.country}.` },
  } : undefined);
}
...
function ProfilePortrait({ name, src }: { name: string; src: string | null }) {
  ...
  if (src) {
    ...
    return <img src={src} alt={`Professional portrait of ${name}`} className="h-full w-full object-cover" />;
  }

  return <div aria-hidden="true" className="flex h-full items-center justify-center bg-brand-100 text-7xl font-semibold text-brand-700">{name.charAt(0)}</div>;
}
  ...
  return (
    <div className="flex-grow">
      <PageHeader
        ...
        title={profile.name}
        titleAr={profile.name}
        subtitle={profile.specialty}
        subtitleAr={profile.specialtyAr || profile.specialty}
      />
        ...
        <div className="container mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid overflow-hidden border border-brand-950/15 bg-bone md:grid-cols-[minmax(17rem,0.72fr)_1.28fr]">
            <div className="relative min-h-[22rem] overflow-hidden bg-brand-50 md:min-h-[36rem]">
              <ProfilePortrait name={profile.name} src={profile.profileImage} />
            </div>
            <div className="p-6 sm:p-9 lg:p-12">
              <p dir="auto" className="text-sm font-semibold uppercase tracking-[0.15em] text-brand-700">
                <LocalizedText en={profile.specialty} ar={profile.specialtyAr || profile.specialty} />
              </p>
              <h2 dir="auto" className="mt-3 font-display text-[clamp(2rem,4vw,3.6rem)] font-semibold leading-none text-brand-950">{profile.name}</h2>
              <p dir="auto" className="mt-3 text-sm text-slate-500">{profile.city}, {profile.country}</p>

              {profile.bio && <p dir="auto" className="mt-8 max-w-2xl text-base leading-8 text-slate-600"><LocalizedText en={profile.bio} ar={profile.bioAr || profile.bio} /></p>}
              ...
              {profile.location && (
                <div className="mt-8 border-t border-brand-950/12 pt-6">
                  ...
                  <p dir="auto" className="mt-3 text-sm leading-7 text-slate-600"><LocalizedText en={profile.location} ar={profile.locationAr || profile.location} /></p>
                </div>
              )}
            ...
            <section className="mt-8 border border-brand-950/15 bg-white p-6 sm:p-8" aria-labelledby="credential-heading">
              <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  ...
                  <h2 id="credential-heading" dir="auto" className="mt-2 text-xl font-semibold text-brand-950">{profile.credentialType}</h2>
                  <dl className="mt-5 grid gap-3 text-sm sm:grid-cols-2 sm:gap-x-10">
                    <div><dt className="text-slate-500"><LocalizedText en="Issuing organization" ar="الجهة المانحة" /></dt><dd dir="auto" className="mt-1 font-medium text-brand-950">{profile.credentialIssuer}</dd></div>
