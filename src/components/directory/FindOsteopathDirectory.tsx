  ...
  return (
    <div className="flex-grow">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
        ...
        {showingApprovedFallback && (
          <div role="status" aria-live="polite" aria-atomic="true" className="mb-6 flex items-start gap-4 rounded-xl border border-gold/35 bg-gold-soft/45 p-5 sm:mb-8 sm:p-6">
            <svg className="mt-0.5 h-5 w-5 shrink-0 text-gold-deep" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-brand-950">
                {isArabic ? 'تحديثات الدليل المباشرة غير متاحة مؤقتًا' : 'Live directory updates are temporarily unavailable'}
              </p>
              <p className="mt-1 text-sm leading-relaxed text-ink-muted">
                {isArabic ? 'نعرض حاليًا ملفات الممارسين المعتمدة المنشورة.' : 'The approved published practitioner profiles are shown below.'}
              </p>
              <button type="button" onClick={retryDirectory} className="mt-2 inline-flex min-h-11 items-center font-medium text-brand-700 underline decoration-gold/70 underline-offset-4 transition-colors hover:text-brand-950">
                {copy.retry}
              </button>
            </div>
          </div>
        ...
        ) : dataUnavailable && osteopaths.length === 0 ? (
