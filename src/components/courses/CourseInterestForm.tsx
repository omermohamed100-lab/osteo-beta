...
export default function CourseInterestForm() {
  ...
  async function submit(event: FormEvent) {
    ...
    setStatus('sending');
    try {
      const { response } = await fetchJsonWithTimeout('/api/course-interests', { signal: requestRef.current.signal, method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, topic, consentNotifications: consent, website: '' }) });
      if (!response.ok) {
        if (response.status === 429) setErrorMessage(submissionFeedback('rate-limit', ar));
      ...
      if (error instanceof RequestTimeoutError) setErrorMessage(submissionFeedback('timeout', ar));
      setStatus('error');
    } finally {
  ...
  return (
    <section className="surface-panel mx-auto max-w-2xl p-6 sm:p-8" aria-labelledby="course-interest-title">
      ...
      <form onSubmit={submit} className="mt-5" aria-busy={status === 'sending'}>
        <fieldset disabled={status === 'sending'} className="min-w-0 space-y-4">
        ...
        {status === 'error' && <p role="alert" className="text-sm font-medium text-red-700">{errorMessage || (ar ? 'تعذر إرسال الطلب. تحقق من البيانات وحاول مرة أخرى.' : 'We could not submit your request. Check the fields and try again.')}</p>}
